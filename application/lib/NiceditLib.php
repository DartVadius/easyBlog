...
class NiceditLib {
  ...
  protected function upload() {
    ...
    if (!$this->error && !$this->file["error"]) {
      ...
      if (!move_uploaded_file($this->file["tmp_name"], $this->docRoot . "/" . $file_name))
  ...
  protected function delete($delete = "") {
    $hash = md5_file($this->docRoot . $delete);
    if (!@unlink($this->docRoot . "/" . $delete))
